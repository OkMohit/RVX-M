...
NEXT_VER_CODE=${NEXT_VER_CODE:-$(date +"%m-%d-%y")}
...
patch_apk() {
	...
	echo "java -jar $RV_CLI_JAR -i custom-branding-icon-revancify --rip-lib armeabi-v7a --rip-lib x86_64 --rip-lib x86 -i website -c -a $stock_input -o $patched_apk -b $RV_PATCHES_JAR --keystore=ks.keystore $patcher_args"
	# shellcheck disable=SC2086
	# --rip-lib is only available in my own revanced-cli builds
	java -jar "$RV_CLI_JAR" -i custom-branding-icon-revancify --rip-lib armeabi-v7a --rip-lib x86_64 --rip-lib x86 -i website -c -a "$stock_input" -o "$patched_apk" -b "$RV_PATCHES_JAR" --keystore=ks.keystore $patcher_args
}
